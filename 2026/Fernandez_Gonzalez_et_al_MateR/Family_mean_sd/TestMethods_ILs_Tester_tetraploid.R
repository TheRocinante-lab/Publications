...
for (rep in 1:nrep) {
  ...
  while (tester %in% c(parents1,parents2)) {
    ...
    Tester <- H_parents[[tester]][1,]
    Tester <- rbind(Tester,Tester)
    Tester <- rbind(Tester,Tester)
  }
  
  testers[[rep]] <- c(parents1, parents2)
  ...
  for (i in 1:n2) {
    ...
    #Cross with tester
    gamete <- create_gamete(phi=phi, F1_selfed, Chromosomes, ChromosomesCentiMorgan = ChromosomesCentiMorgan)
    gameteT <- create_gamete(phi=phi, Tester, Chromosomes, ChromosomesCentiMorgan = ChromosomesCentiMorgan)
  ...
  colnames(MarkersT) <- colnames(MarkersF1)
  ...
  out <- calculateFamilyValues(P=P,
                               ...
                               SigmaT=create_LD_list(rbind(Markers[,QTL], MarkersT[,QTL, drop = F]), parametrization = "Genotypic", phi=phi, markereffects=markereffects$YLD[QTL], markereffects_d = NULL, Chromosomes = lapply(Chromosomes_names, function(x){x[x %in% QTL]})),
                               H_parents = NULL,
                               H_testers = NULL,
                               c_list = lapply(c_list, function(x){x[rownames(x) %in% QTL,colnames(x) %in% QTL]}),
                               ncores = 1)
  ...
  out <- calculateFamilyValues(P=P,
                               ...
                               SigmaT=create_LD_list(rbind(Markers[,QTL], MarkersT[,QTL, drop = F]), parametrization = "Genotypic", phi=phi, markereffects=markereffects_hat[QTL], markereffects_d = NULL, Chromosomes = lapply(Chromosomes_names, function(x){x[x %in% QTL]})),
                               H_parents = NULL,
                               H_testers = NULL,
                               c_list = lapply(c_list, function(x){x[rownames(x) %in% QTL,colnames(x) %in% QTL]}),
                               ncores = 1)
  ...
  out <- calculateFamilyValues(P=P,
                               ...
                               SigmaT=create_LD_list(rbind(Markers[,QTL], MarkersT[,QTL, drop = F]), parametrization = "Genotypic", phi=phi, markereffects=markereffects_hat_0.5[QTL], markereffects_d = NULL, Chromosomes = lapply(Chromosomes_names, function(x){x[x %in% QTL]})),
                               H_parents = NULL,
                               H_testers = NULL,
                               c_list = lapply(c_list, function(x){x[rownames(x) %in% QTL,colnames(x) %in% QTL]}),
                               ncores = 1)
  ...
  out <- calculateFamilyValues(P=P,
                               ...
                               SigmaT=create_LD_list(rbind(Markers[,QTL], MarkersT[,QTL, drop = F]), parametrization = "Genotypic", phi=phi, markereffects=markereffects$YLD[QTL], markereffects_d = markereffects_d$YLD[QTL], Chromosomes = lapply(Chromosomes_names, function(x){x[x %in% QTL]})),
                               H_parents = NULL,
                               H_testers = NULL,
                               c_list = lapply(c_list, function(x){x[rownames(x) %in% QTL,colnames(x) %in% QTL]}),
                               ncores = 1)
  ...
  out <- calculateFamilyValues(P=P,
                               ...
                               SigmaT=create_LD_list(rbind(Markers[,QTL], MarkersT[,QTL, drop = F]), parametrization = "Genotypic", phi=phi, markereffects=markereffects_hat[QTL], markereffects_d = markereffects_d_hat[QTL], Chromosomes = lapply(Chromosomes_names, function(x){x[x %in% QTL]})),
                               H_parents = NULL,
                               H_testers = NULL,
                               c_list = lapply(c_list, function(x){x[rownames(x) %in% QTL,colnames(x) %in% QTL]}),
                               ncores = 1)
  ...
  out <- calculateFamilyValues(P=P,
                               ...
                               SigmaT=create_LD_list(rbind(Markers[,QTL], MarkersT[,QTL, drop = F]), parametrization = "Genotypic", phi=phi, markereffects=markereffects_hat_0.5[QTL], markereffects_d = markereffects_d_hat_0.5[QTL], Chromosomes = lapply(Chromosomes_names, function(x){x[x %in% QTL]})),
                               H_parents = NULL,
                               H_testers = NULL,
                               c_list = lapply(c_list, function(x){x[rownames(x) %in% QTL,colnames(x) %in% QTL]}),
                               ncores = 1)
...
results_ILs_Tester_tetraploiod <- list(additive_values_list=additive_values_list,
...
save(results_ILs_Tester_tetraploiod, file = paste0("./ResultsILs_Tester_tetraploid_n_", n,".RData"))
