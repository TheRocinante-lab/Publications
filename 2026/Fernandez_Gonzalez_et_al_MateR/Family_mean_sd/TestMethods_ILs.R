...
for (rep in 1:nrep) {
  ...
  #Simple mating does not require haplotypes for only additive!!
  ...
  means[["SM"]][["additive"]][[rep]] <- out[[1]]$Mean
  ...
  sd_sigma_SM_hat <- c(sd_sigma_SM_hat, out[[1]]$sd)
  means[["SM_hat"]][["additive"]][[rep]] <- out[[1]]$Mean
  ...
  sd_sigma_SM_hat_0.5 <- c(sd_sigma_SM_hat_0.5, out[[1]]$sd)
  means[["SM_hat_0.5"]][["additive"]][[rep]] <- out[[1]]$Mean
  ...
  sd_sigma_SM_realistic <- c(sd_sigma_SM_realistic, out[[1]]$sd)
  means[["SM_realistic"]][["additive"]][[rep]] <- out[[1]]$Mean
...
results_ILs <- list(additive_values_list=additive_values_list,
                     parents=parents,
                     means=means,
...
save(results_ILs, file = paste0("./ResultsILs_n_", n,".RData"))
...
#SimpleMating
sqrt(mean( (sd_additive-sd_sigma_SM)^2 ))/mean(sd_additive)*100
...
#SimpleMating
sqrt(mean( (sd_additive-sd_sigma_SM_hat)^2 ))/mean(sd_additive)*100
...
#SimpleMating
sqrt(mean( (sd_additive-sd_sigma_SM_hat_0.5)^2 ))/mean(sd_additive)*100
...
#SimpleMating
sqrt(mean( (sd_additive-sd_sigma_SM_realistic)^2 ))/mean(sd_additive)*100
